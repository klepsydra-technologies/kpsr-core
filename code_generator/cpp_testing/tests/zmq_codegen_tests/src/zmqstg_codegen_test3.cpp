...
#include <klepsydra/codegen/cereal/composition_type_serializer.h>

TEST(KpsrZMQCodegeTest, compositionTypeMapperTest)
{
    ...
    kpsr::zmq_mdlw::FromZmqChannel<std::string> *fromZMQProvider =
        _fromZmqMiddlewareProvider
            .getJsonFromMiddlewareChannel<kpsr::codegen::CompositionType>(subscriber, 10);
    fromZMQProvider->start();

    kpsr::Publisher<kpsr::codegen::CompositionType> *kpsrPublisher =
        toZMQProvider.getJsonToMiddlewareChannel<kpsr::codegen::CompositionType>(topic, 0);

    kpsr::EventEmitterMiddlewareProvider<kpsr::codegen::CompositionType> basicProvider(nullptr,
                                                                                       "test",
                                                                                       0,
                                                                                       nullptr,
                                                                                       nullptr);
    ...
    while (cacheListener.counter < 1) {
        ...
        event.newEnumVectorPointer = {std::shared_ptr<kpsr::codegen::NewEnum>(
        ...
        event.oldEnumVectorPointer = {std::shared_ptr<kpsr::codegen::OldEnum>(
        ...
        event.positionVector = {kpsr::geometry::Vector3(seq++, 0.1, 0.2, 0.3),
                                kpsr::geometry::Vector3(seq++, 1.1, 1.2, 1.3)};
        event.positionVectorPointer = {std::shared_ptr<kpsr::geometry::Vector3>(
        ...
        event.quaternionVector = {kpsr::codegen::Vector4(0.1, 0.2, 0.3, 0.4),
                                  kpsr::codegen::Vector4(1.1, 1.2, 1.3, 1.4)};
        event.quaternionVectorPointer = {std::shared_ptr<kpsr::codegen::Vector4>(
