...
using Implementations =
    ::testing::Types<std_msgs::Bool, std_msgs::Int32, std_msgs::Int64, std_msgs::Float32>;
...
TYPED_TEST(KpsrRosIdentityTest, nominalCasePublish)
{
    ...
    auto subscriber = nodeHandle.subscribe<TypeParam>(topicName,
                                                      100,
                                                      boost::bind(callback<TypeParam>,
                                                                  _1,
                                                                  itemToSend));
    ...
    auto kpsrPublisher = toRosProvider.getToMiddlewareChannel<TypeParam>(topicName,
                                                                         1,
                                                                         nullptr,
                                                                         publisher);
...
TEST(KpsrIdentityTest, nominalCaseStringPublisher)
{
    ...
    auto subscriber = nodeHandle.subscribe<std_msgs::String>(topicName,
                                                             100,
                                                             boost::bind(callback<std_msgs::String>,
                                                                         _1,
                                                                         itemToSend));
    ...
    // Create klepsydra publisher.
    auto kpsrPublisher = toRosProvider.getToMiddlewareChannel<std_msgs::String>(topicName,
                                                                                1,
                                                                                nullptr,
                                                                                publisher);
...
TYPED_TEST(KpsrRosIdentityTest, nominalCaseSubscribe)
{
    ...
    kpsr::mem::BasicMiddlewareProvider<TypeParam>
    ...
    fromRosProvider.registerToTopic<TypeParam>(topicName.c_str(),
                                               1,
...
TEST(KpsrIdentityTest, nominalCaseSubscribeString)
{
    ...
    kpsr::mem::BasicMiddlewareProvider<std_msgs::String>
        safeQueueProvider(nullptr, "test", 8, 0, nullptr, nullptr, false);

    // Create a handler to the ros provider using klepsydra.
    kpsr::ros_mdlw::FromRosMiddlewareProvider fromRosProvider(nodeHandle);

    fromRosProvider.registerToTopic<std_msgs::String>(topicName.c_str(),
                                                      1,
                                                      safeQueueProvider.getPublisher());
    safeQueueProvider.start();
