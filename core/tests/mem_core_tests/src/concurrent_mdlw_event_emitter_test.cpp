...
public:
    ...
    ConcurrentSQTestEvent() { ConcurrentSQTestEvent::emptyConstructorInvokations++; }
...
public:
    ...
    ConcurrentSQTestNewEvent() { ConcurrentSQTestNewEvent::emptyConstructorInvokations++; }
...
TEST(ConcurrentEventEmitterTest, SingleEventEmitterTopic)
{
    ...
    kpsr::mem::ConcurrentMiddlewareProvider<ConcurrentSQTestEvent>
        provider(nullptr, "event", 4, 0, nullptr, nullptr, false, 1000);
    provider.start();
...
TEST(ConcurrentEventEmitterTest, WithObjectPoolNoFailures)
{
    ...
    kpsr::mem::ConcurrentMiddlewareProvider<ConcurrentSQTestEvent>
        provider(nullptr, "event", queueSize, poolSize, nullptr, nullptr, false, 1000);
...
TEST(ConcurrentEventEmitterTest, WithObjectPoolWithFailuresNonBlocking)
{
    ...
    const int queueSize = 4;
    kpsr::mem::ConcurrentMiddlewareProvider<ConcurrentSQTestEvent>
        provider(nullptr, "event", queueSize, poolSize, nullptr, nullptr, true, 1000);
    ASSERT_EQ(ConcurrentSQTestEvent::emptyConstructorInvokations, poolSize);
...
TEST(ConcurrentEventEmitterTest, TransformForwaringTestNoPool)
{
    ...
    kpsr::mem::ConcurrentMiddlewareProvider<ConcurrentSQTestEvent>
        provider(nullptr, "event", queueSize, poolSize, nullptr, nullptr, false, 1000);
    kpsr::mem::ConcurrentMiddlewareProvider<ConcurrentSQTestNewEvent>
        newProvider(nullptr, "newEvent", queueSize, poolSize, nullptr, nullptr, false, 1000);

    provider.start();
    newProvider.start();

    std::function<void(const ConcurrentSQTestEvent &, ConcurrentSQTestNewEvent &)> transformFunction =
        [](const ConcurrentSQTestEvent &src, ConcurrentSQTestNewEvent &dest) {
