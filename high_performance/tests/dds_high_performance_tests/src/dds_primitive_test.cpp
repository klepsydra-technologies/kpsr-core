...
enum GreetEnum { HOLA = 0, HELLO, CIAO, HALLO };
...
public:
    ...
    void start()
    {
        std::function<void(GreetEnum)> enumListenerFunction =
            std::bind(&PrimitiveSubscriberService::onEnumDataReceived, this, std::placeholders::_1);
        this->_enumSubscriber->registerListener("primitive_subscriber_service",
                                                enumListenerFunction);

        std::function<void(unsigned char)> octetListenerFunction =
            std::bind(&PrimitiveSubscriberService::onOctetDataReceived, this, std::placeholders::_1);
        this->_octetSubscriber->registerListener("primitive_subscriber_service",
                                                 octetListenerFunction);

        std::function<void(int)> intListenerFunction =
            std::bind(&PrimitiveSubscriberService::onIntDataReceived, this, std::placeholders::_1);
        this->_intSubscriber->registerListener("primitive_subscriber_service", intListenerFunction);

        std::function<void(bool)> boolListenerFunction =
            std::bind(&PrimitiveSubscriberService::onBoolDataReceived, this, std::placeholders::_1);
        this->_boolSubscriber->registerListener("primitive_subscriber_service",
                                                boolListenerFunction);

        std::function<void(double)> doubleListenerFunction =
            std::bind(&PrimitiveSubscriberService::onDoubleDataReceived,
                      this,
                      std::placeholders::_1);
        this->_doubleSubscriber->registerListener("primitive_subscriber_service",
                                                  doubleListenerFunction);

        std::function<void(float)> floatListenerFunction =
            std::bind(&PrimitiveSubscriberService::onFloatDataReceived, this, std::placeholders::_1);
        this->_floatSubscriber->registerListener("primitive_subscriber_service",
                                                 floatListenerFunction);

        std::function<void(std::string)> stringListenerFunction =
            std::bind(&PrimitiveSubscriberService::onStringDataReceived,
                      this,
                      std::placeholders::_1);
        this->_stringSubscriber->registerListener("primitive_subscriber_service",
                                                  stringListenerFunction);
    }
...
private:
    void onEnumDataReceived(const GreetEnum &eventData) { enumData = eventData; }

    void onOctetDataReceived(const unsigned char &eventData) { charData = eventData; }

    void onIntDataReceived(const int &eventData) { intData = eventData; }

    void onBoolDataReceived(const bool &eventData) { boolData = eventData; }

    void onDoubleDataReceived(const double &eventData) { doubleData = eventData; }

    void onFloatDataReceived(const float &eventData) { floatData = eventData; }

    void onStringDataReceived(const std::string &eventData) { stringData = eventData; }
...
TEST(DDSStgCoreTests, PubSubTest)
{
    ...
    kpsr::Publisher<GreetEnum> *enumPublisher =
        provider.getToMiddlewareChannel<GreetEnum, kpsr_dds_serialization::LongData>("enumData",
                                                                                     0,
                                                                                     nullptr,
                                                                                     &enumDataWriter);

    dds::topic::Topic<kpsr_dds_serialization::OctetData> octetDataTopic(dp, "octetData");
    dds::pub::DataWriter<kpsr_dds_serialization::OctetData> octetDataWriter(publisher,
                                                                            octetDataTopic);
    kpsr::Publisher<unsigned char> *octetDataPublisher =
        provider.getToMiddlewareChannel<unsigned char, kpsr_dds_serialization::OctetData>(
            "octetData", 0, nullptr, &octetDataWriter);
    ...
    kpsr::Publisher<int> *longDataPublisher =
        provider.getToMiddlewareChannel<int, kpsr_dds_serialization::LongData>("longData",
                                                                               0,
                                                                               nullptr,
                                                                               &longDataWriter);
    ...
    kpsr::Publisher<bool> *boolDataPublisher =
        provider.getToMiddlewareChannel<bool, kpsr_dds_serialization::BoolData>("boolData",
                                                                                0,
                                                                                nullptr,
                                                                                &boolDataWriter);

    dds::topic::Topic<kpsr_dds_serialization::DoubleData> doubleDataTopic(dp, "doubleData");
    dds::pub::DataWriter<kpsr_dds_serialization::DoubleData> doubleDataWriter(publisher,
                                                                              doubleDataTopic);
    kpsr::Publisher<double> *doubleDataPublisher =
        provider
            .getToMiddlewareChannel<double, kpsr_dds_serialization::DoubleData>("doubleData",
                                                                                0,
                                                                                nullptr,
                                                                                &doubleDataWriter);

    dds::topic::Topic<kpsr_dds_serialization::FloatData> floatDataTopic(dp, "floatData");
    dds::pub::DataWriter<kpsr_dds_serialization::FloatData> floatDataWriter(publisher,
                                                                            floatDataTopic);
    kpsr::Publisher<float> *floatDataPublisher =
        provider.getToMiddlewareChannel<float, kpsr_dds_serialization::FloatData>("floatData",
                                                                                  0,
                                                                                  nullptr,
                                                                                  &floatDataWriter);
    ...
    kpsr::Publisher<std::string> *stringDataPublisher =
        provider.getToMiddlewareChannel<std::string, kpsr_dds_serialization::StringData>(
            "stringData", 0, nullptr, &stringDataWriter);
    ...
    ddsProvider.registerToTopic("enumData",
                                &enumDataReader,
                                true,
                                eventloopProvider.getPublisher<GreetEnum>("enumData",
                                                                          0,
                                                                          nullptr,
                                                                          nullptr));

    dds::sub::DataReader<kpsr_dds_serialization::OctetData> octetDataReader(subscriber,
                                                                            octetDataTopic);
    ddsProvider.registerToTopic("octetData",
                                &octetDataReader,
                                true,
                                eventloopProvider.getPublisher<unsigned char>("octetData",
                                                                              0,
                                                                              nullptr,
                                                                              nullptr));

    dds::sub::DataReader<kpsr_dds_serialization::LongData> longDataReader(subscriber, longDataTopic);
    ddsProvider.registerToTopic("longData",
                                &longDataReader,
                                true,
                                eventloopProvider.getPublisher<int>("longData", 0, nullptr, nullptr));

    dds::sub::DataReader<kpsr_dds_serialization::BoolData> boolDataReader(subscriber, boolDataTopic);
    ddsProvider
        .registerToTopic("boolData",
                         &boolDataReader,
                         true,
                         eventloopProvider.getPublisher<bool>("boolData", 0, nullptr, nullptr));

    dds::sub::DataReader<kpsr_dds_serialization::DoubleData> doubleDataReader(subscriber,
                                                                              doubleDataTopic);
    ddsProvider
        .registerToTopic("doubleData",
                         &doubleDataReader,
                         true,
                         eventloopProvider.getPublisher<double>("doubleData", 0, nullptr, nullptr));

    dds::sub::DataReader<kpsr_dds_serialization::FloatData> floatDataReader(subscriber,
                                                                            floatDataTopic);
    ddsProvider
        .registerToTopic("floatData",
                         &floatDataReader,
                         true,
                         eventloopProvider.getPublisher<float>("floatData", 0, nullptr, nullptr));

    dds::sub::DataReader<kpsr_dds_serialization::StringData> stringDataReader(subscriber,
                                                                              stringDataTopic);
    ddsProvider.registerToTopic("stringData",
                                &stringDataReader,
                                true,
                                eventloopProvider.getPublisher<std::string>("stringData",
                                                                            0,
                                                                            nullptr,
                                                                            nullptr));

    PrimitiveSubscriberService
        primitiveSubscriberService(eventloopProvider.getSubscriber<GreetEnum>("enumData"),
    ...
    for (int i = 0; i < 100; i++) {
        ...
        ASSERT_EQ((GreetEnum)(primitivePublisherService._seq % 4),
                  primitiveSubscriberService.enumData);
        ...
        ASSERT_DOUBLE_EQ((primitivePublisherService._seq * 10.0),
                         primitiveSubscriberService.doubleData);
        ASSERT_FLOAT_EQ((primitivePublisherService._seq * 100.0),
                        primitiveSubscriberService.floatData);
        ASSERT_EQ((primitivePublisherService._greetings[primitivePublisherService._seq % 4]),
                  primitiveSubscriberService.stringData);
    }
